// src/database/seed-knust-hostels.ts
//
// Run: npx ts-node -r tsconfig-paths/register src/database/seed-knust-hostels.ts
//
// Seeds 30 KNUST hostels + 30 owners + 1 super-owner.
// Does NOT touch the existing 4 hostels from seed.ts.
...
interface RoomDef {
  num: string;
  type: '1-in-a-room' | '2-in-a-room' | '4-in-a-room';
  price: number;
  beds: string[];
  ac: boolean;
  bath: boolean;
...
interface HostelDef {
  ...
  desc: string;
  amenities: string[];
  config: 'doubles_only' | 'doubles_singles' | 'doubles_quads';
...
async function seed() {
  ...
  // Super-owner (you)
  const [superOwner] = await q(
    `INSERT INTO users (full_name, email, password_hash, role, email_verified, phone_verified, id_verified, profile_completed)
     VALUES ($1, $2, $3, 'OWNER', true, true, true, true)
     ON CONFLICT (email) DO UPDATE SET full_name = EXCLUDED.full_name
     RETURNING id`,
    ['StayNest Admin', 'super@staynest.com', superHash],
  );
  ...
  for (const h of HOSTELS) {
    ...
    // Create owner for this hostel
    const [ow] = await q(
      `INSERT INTO users (full_name, email, password_hash, role, email_verified, phone_verified, id_verified, profile_completed)
       VALUES ($1, $2, $3, 'OWNER', true, true, true, true)
       ON CONFLICT (email) DO UPDATE SET full_name = EXCLUDED.full_name
       RETURNING id`,
      [`${h.name} Management`, ownerEmail, ownerHash],
    );

    // Insert hostel
    const [hostel] = await q(
      `INSERT INTO hostels (owner_id, name, description, address, city, status, verified, gender_policy, location, university, latitude, longitude)
       VALUES ($1, $2, $3, $4, $5, 'ACTIVE', true, $6, ST_SetSRID(ST_MakePoint($7, $8), 4326), 'KNUST', $9, $10)
       ON CONFLICT DO NOTHING
       RETURNING id`,
      [ow.id, h.name, h.desc, h.address, 'Kumasi', h.gender, h.lng, h.lat, h.lat, h.lng],
    );
    ...
    // Generate buildings, floors, rooms, beds
    ...
    for (let b = 0; b < buildingCount; b++) {
      ...
      for (let f = 0; f < floorCount; f++) {
        ...
        for (let r = 0; r < roomsPerFloor; r++) {
          let roomType: '1-in-a-room' | '2-in-a-room' | '4-in-a-room';
          let bedCount: number;
          ...
          const [room] = await q(
            `INSERT INTO rooms (floor_id, number, type, price_pesewas, has_ac, has_private_bath, description)
             VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`,
            [floor.id, roomNum, roomType, price, hasAC, hasBath, `${roomType} in ${h.name}`],
          );
