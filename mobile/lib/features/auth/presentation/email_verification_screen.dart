// features/auth/presentation/email_verification_screen.dart
...
class _EmailVerificationScreenState
  ...
  Future<void> _loadStatusAndSendOtp() async {
    try {
      ...
      if (status.email != null && status.email!.isNotEmpty) {
        setState(() => _emailDisplay = status.email!);
      }

      await repo.sendOtp(type: 'EMAIL');
      if (!mounted) return;
      _startResendTimer();
  ...
  Future<void> _submit() async {
    ...
    try {
      final repo = ref.read(authRepositoryProvider);
      await repo.verifyOtp(type: 'EMAIL', code: _fullCode);
      if (!mounted) return;
